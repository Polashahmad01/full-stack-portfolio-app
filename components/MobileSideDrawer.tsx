...
export default function MobileSideDrawer({
  ...
  return (
    <section>
      ...
      <article
        className={`fixed top-0 left-0 z-50 w-[50vw] bg-teal-50/70 text-black h-screen transition-transform ease-in-out duration-300 md:hidden ${
        ...
        <div className="m-4 flex justify-center items-center h-full">
          <ul className="font-normal flex flex-col gap-2">
            <li className="group" onClick={onClose}>
              <Link
                className="flex items-center gap-2 px-4 py-1 border rounded-xl transition-all hover:border-blue-700"
                href="#intro">
                <FaHome className="w-4 h-4 transition-all group-hover:text-blue-700" />
                <span className="transition-all group-hover:text-blue-700">
                  Home
                </span>
              </Link>
            </li>
